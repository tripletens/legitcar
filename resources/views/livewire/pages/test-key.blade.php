<div class="flex flex-col">
    ...
    <h1 class="p-auto pb-0 text-lg mx-2">Test Key</h1>

    <div class="flex flex-row p-auto space-x-3">
        <input type={{ $input_type }} readonly wire:load="fetch_test_key" wire:model="test_key" id="test_key"
            class="border-slate-400 border-2 rounded-lg h-10 m-2 bg-slate-200 w-3/4" value="{{ $test_key }}" />
        ...
        <button onclick="copy_test_key()" class="m-2 p-3 rounded-full bg-slate-200 cursor-pointer">
            <img class="h-5" src="{{ asset('images/content-copy.svg') }}" title="Copy" />
        </button>

        <button wire:click="create_test_key" wire:confirm="Are you sure you want to renew api key?"
...
<script>
    function copy_test_key() {
        // Get the text field
        var copyText = document.getElementById("test_key");
        ...
        alert("Copied the Test Apikey successfully - " + copyText.value);
    }
</script>
